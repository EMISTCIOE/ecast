...
type Notice = { id: string; title: string; content: string; created_at: string; audience: string; published_by_username: string };
...
export default function MemberDashboard() {
  ...
  const createNotice = async (e: React.FormEvent) => {
    ...
    setMsg('');
    try {
      await createNoticeApi({ title: nTitle, content: nContent, audience: nAudience });
        ...
        <div className="md:ml-64 p-6 pt-24">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                >
                  ...
                  <p className="text-sm text-gray-400 mt-2">Post announcements</p>
                </button>
                ...
                >
                  ...
                  <p className="text-sm text-gray-400 mt-2">Share your thoughts</p>
                </button>
                ...
                >
                  ...
                  <p className="text-sm text-gray-400 mt-2">Organize activities</p>
                </button>
              </div>
              ...
              <div className="bg-gray-800/50 backdrop-blur p-6 rounded-xl border border-gray-700 shadow-xl">
                ...
                <div className="space-y-3">
                  {sorted.slice(0, 5).map(n => (
                    <div key={n.id} className="bg-gray-900/50 p-4 rounded-lg border border-gray-700">
                      <h4 className="font-semibold text-purple-300">{n.title}</h4>
                      <p className="text-sm text-gray-400 mt-1">
                        {new Date(n.created_at).toLocaleDateString()} • {n.audience} • by {n.published_by_username}
                      </p>
                      <p className="text-sm text-gray-300 mt-2 line-clamp-2">{n.content}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeSection === 'notice' && (
            <div className="max-w-2xl">
              ...
              <form onSubmit={createNotice} className="bg-gray-800/50 backdrop-blur p-8 rounded-xl border border-gray-700 shadow-xl space-y-5">
                {msg && (
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-300">Audience</label>
                  <select 
                    className="w-full p-3 bg-gray-900 border border-gray-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition" 
                    value={nAudience} 
          ...
          {activeSection === 'blog' && (
            <div className="max-w-2xl">
              ...
              <form onSubmit={createBlog} className="bg-gray-800/50 backdrop-blur p-8 rounded-xl border border-gray-700 shadow-xl space-y-5">
                {msg && (
                  <div className={`p-4 rounded-lg ${msg.includes('Failed') ? 'bg-red-900/50 text-red-300' : 'bg-green-900/50 text-green-300'}`}>
                    {msg}
                  </div>
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-300">Content</label>
                  <textarea 
                    className="w-full p-3 bg-gray-900 border border-gray-700 rounded-lg h-32 focus:ring-2 focus:ring-pink-500 focus:border-transparent transition" 
          ...
          {activeSection === 'project' && (
            <div className="max-w-2xl">
              ...
              <form onSubmit={createProject} className="bg-gray-800/50 backdrop-blur p-8 rounded-xl border border-gray-700 shadow-xl space-y-5">
                {msg && (
                  <div className={`p-4 rounded-lg ${msg.includes('Failed') ? 'bg-red-900/50 text-red-300' : 'bg-green-900/50 text-green-300'}`}>
                    {msg}
                  </div>
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-300">Live URL (optional)</label>
                  <input 
                    className="w-full p-3 bg-gray-900 border border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition" 
                    placeholder="https://..." 
                    value={pLive} 
          ...
          {activeSection === 'event' && (
            <div className="max-w-2xl">
              ...
              <form onSubmit={createEvent} className="bg-gray-800/50 backdrop-blur p-8 rounded-xl border border-gray-700 shadow-xl space-y-5">
                {msg && (
                  <div className={`p-4 rounded-lg ${msg.includes('Failed') ? 'bg-red-900/50 text-red-300' : 'bg-green-900/50 text-green-300'}`}>
                    {msg}
                  </div>
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-300">Event Image</label>
                  <input 
