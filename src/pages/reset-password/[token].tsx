...
export default function ResetPasswordTokenPage() {
  ...
  const confirmReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data?.detail || "Invalid or expired reset link. Please request a new one.");
      }
    } catch (err) {
  ...
  const displayName = claims?.name || claims?.uname || claims?.email || "your account";
  const avatarUrl = (() => {
  ...
  const expText = claims?.exp ? new Date((claims.exp || 0) * 1000).toLocaleString() : null;
          ...
          <div className="bg-gray-900/50 backdrop-blur-xl p-8 rounded-2xl shadow-2xl border border-gray-800/50 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="text-center space-y-2">
              ...
              <div className="flex items-center justify-center gap-3">
                {avatarUrl ? (
                  <img src={avatarUrl} alt="avatar" className="w-12 h-12 rounded-full object-cover border border-gray-700" />
                ) : (
              ...
              <p className="text-gray-400 text-sm">
                Resetting password for <span className="text-gray-200 font-semibold">{displayName}</span>
              </p>
              {claims?.email && (
              ...
              {claims?.exp && (
                <p className="text-xs text-gray-400">
                  Expires {isExpired ? "now" : `in ${expiresIn}`} {expText ? `(${expText})` : ""}
                </p>
              )}
              {isExpired && (
                <div className="mt-2 bg-red-500/10 border border-red-500/50 text-red-400 px-4 py-2 rounded-lg inline-flex items-center gap-2">
                  <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.257 3.099C8.734 2.42 9.344 2 10 2c.656 0 1.266.42 1.743 1.099l6 8.5A2 2 0 0115.999 14H4.001a2 2 0 01-1.744-2.401l6-8.5z" clipRule="evenodd"/></svg>
            ...
            <form onSubmit={confirmReset} className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300 block">New Password</label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    required
                  />
                  <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-gray-300 transition-colors">
                    {showNewPassword ? (
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    required
                  />
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-gray-300 transition-colors">
                    {showConfirmPassword ? (
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                    ) : (
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
              ...
              <button
                type="submit"
                disabled={isLoading || !newPassword || newPassword !== confirmPassword || isExpired}
            ...
            <div className="text-center pt-2">
              <a href="/login" className="text-sm text-blue-400 hover:text-blue-300 transition-colors inline-flex items-center gap-1 group">
